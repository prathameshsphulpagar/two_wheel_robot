...
{ // MyFile.open("lidar_csv.ods",std::ios::app);
	...
	{ // for loop for reading the angle and its value
		...
		if (distance_f32.data <= 1.000 && distance_f32.data >= 0.0010) // if(distance_avg <= 0.15)// && quality_f64.data == 1)
		{
...
void timer_function()
{
	...
	{ // condition
		...
		ROS_WARN_STREAM("lidar right is get killed");
		system("rosnode kill rplidar_node"); // killer command
	}
}
int main(int argc, char **argv)
{
	ros::init(argc, argv, "rplidar_client_right_node"); // creating node
	ros::NodeHandle n;
	ROS_INFO("Lidar Client right code");
	...
	ros::Subscriber sub = n.subscribe<sensor_msgs::LaserScan>("/scan_right", 1000, scanCallback); // taking scan lidar data from rplidar pakage
	//  sensing_stoping_pub = n.advertise<std_msgs::Int32> ("sensing_stoping_lidar_right_topic", 1000);
	ros::Publisher pub_sensing_stoping = n.advertise<geometry_msgs::Twist>("sensing_stoping_lidar_right_topic", 10);
	while (ros::ok())
	{
