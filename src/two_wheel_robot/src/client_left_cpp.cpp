...
void timer_function()
{
	...
	{ // condition
		...
		ROS_WARN_STREAM("lidar left is get killed");
		system("rosnode kill rplidar_node"); // killer command
	}
}
int main(int argc, char **argv)
{
	ros::init(argc, argv, "rplidar_client_left_node"); // creating node
	ros::NodeHandle n;
	ROS_INFO("Lidar Client left code");
	...
	ros::Subscriber sub = n.subscribe<sensor_msgs::LaserScan>("/scan_left", 1, scanCallback); // taking scan lidar data from rplidar pakage
	// sensing_stoping_pub = n.advertise<std_msgs::Int32> ("sensing_stoping_lidar_left_topic", 1000);
	ros::Publisher pub_sensing_stoping = n.advertise<geometry_msgs::Twist>("sensing_stoping_lidar_left_topic", 1);
	while (ros::ok())
	{
